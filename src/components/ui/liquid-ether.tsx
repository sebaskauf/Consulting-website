...
`;
    ...
    class Simulation {
      ...
      update() {
        ...
        if (this.externalForce && this.externalForce.mouse) {
          const time = Common.time;
          const ambientForce = 0.85; // DRASTICALLY increased for visible colors
          // Add multiple wave frequencies for more dynamic movement
          const baseX = Math.sin(time * 0.3) * ambientForce + Math.sin(time * 0.7) * (ambientForce * 0.5);
          const baseY = Math.cos(time * 0.4) * ambientForce + Math.cos(time * 0.9) * (ambientForce * 0.5);

          const uniforms = (this.externalForce.mouse.material as any).uniforms;
          const currentForceX = (Mouse.diff.x / 2) * this.options.mouse_force;
          const currentForceY = (Mouse.diff.y / 2) * this.options.mouse_force;

          // Combine ambient force with mouse force
          uniforms.force.value.set(
            currentForceX + baseX,
            currentForceY + baseY
          );
          // Smaller, more refined waves but still visible
          uniforms.scale.value.set(this.options.cursor_size * 3, this.options.cursor_size * 3);
        }
